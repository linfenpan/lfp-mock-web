...
class PatBuilder extends Builder {
  static readFile(filePath, options) {
    options = Object.assign({ compileWithExpand: true/*是否对expand语法进行编译*/ }, options || {});

    const conf = require('../../../index').config;
    let content = util.readFile(filePath, conf.CODE);

    if (options.compileWithExpand) {
      // 替换掉 <!--#CGIEXT expand xxx --> 的内容
  ...
  static renderPatTemplate(filepath, data, readOptions) {
    let html = this.readFile(filepath, readOptions);
    html = toNunjucks(html);

    let dataPat = Object.assign({}, defaultOptions, data || {});
    return template.renderString(html, Object.assign({ __ctx__: dataPat }, dataPat));
  }
  ...
  static *build(options) {
    ...
    if (filepath) {
      ...
      try {
        let result = this.renderPatTemplate(filepath, data);
        // 然后，再走一轮 jinja 的模板..
        ...
        try {
          ...
          thenable.resolve({
            code: 500,
            content: `<html><head></head><body><h1>jinja2解析失败</h1>${result}</body></html>`
          });
        }
      } catch (e) {
        console.error(e);
